...
/-- The same scanner, with the locally transparent decision procedure exposed. -/
...
/-- `scan` cannot depend on the choice of `Decidable` witness. -/
...
example : transparentScan first01.x first01.y first01.z =
    some ⟨0, 1, .yz, 1, 1⟩ := by
  decide
...
def diagnosedBridgeFirst01 := certifiedFiveToFour first01

example : diagnosedBridgeFirst01.scan.candidate = ⟨0, 1, .yz, 1, 1⟩ := by
  unfold diagnosedBridgeFirst01 certifiedFiveToFour FieldNativePairBridge.certifiedScanGauge
