...
private theorem exec_context_disjoint {I : DisplayedF3PairTriple a b c}
    (C : State F3 a b c) (hC : Disjoint C (displayedCarrier I)) :
    Disjoint C (stateUnion I.sourceState I.targetState) := by
  rw [displayedCarrier_eq_spec] at hC
  exact hC

private theorem exec_before_source {I : DisplayedF3PairTriple a b c}
    (B : CertifiedFiveToFour I) (C : State F3 a b c)
    (hs : schedule B C = .beforeResidual) :
    outerSource B ⊆ fullStart (I := I) C := by
  rw [outerSource_eq_spec, fullStart_eq_spec]
  exact source_subset_initial B C (before_source_q B C hs)

private theorem exec_before_fresh {I : DisplayedF3PairTriple a b c}
    (B : CertifiedFiveToFour I) (C : State F3 a b c)
    (hC : Disjoint C (displayedCarrier I))
    (hs : schedule B C = .beforeResidual) :
    Disjoint (outerTarget B)
      (ComputedState.difference (fullStart (I := I) C) (outerSource B)) := by
  rw [outerTarget_eq_spec, ComputedState.difference_eq_spec,
    fullStart_eq_spec, outerSource_eq_spec]
  exact target_fresh_initial B C (exec_context_disjoint C hC) (before_target_q B C hs)

private theorem exec_after_target {I : DisplayedF3PairTriple a b c}
    (B : CertifiedFiveToFour I) (C : State F3 a b c)
    (hs : schedule B C = .afterResidual) :
    outerTarget B ⊆ fullFinish (I := I) C := by
  rw [outerTarget_eq_spec, fullFinish_eq_spec]
  exact target_subset_final B C (after_target_q B C hs)

private theorem exec_after_fresh {I : DisplayedF3PairTriple a b c}
    (B : CertifiedFiveToFour I) (C : State F3 a b c)
    (hC : Disjoint C (displayedCarrier I))
    (hs : schedule B C = .afterResidual) :
    Disjoint (outerSource B)
      (ComputedState.difference (fullFinish (I := I) C) (outerTarget B)) := by
  rw [outerSource_eq_spec, ComputedState.difference_eq_spec,
    fullFinish_eq_spec, outerTarget_eq_spec]
  exact source_fresh_final B C (exec_context_disjoint C hC) (after_source_q B C hs)
...
def execCompose {I : DisplayedF3PairTriple a b c}
  ...
  | beforeResidual =>
      let first := outerStep B (fullStart (I := I) C)
        (exec_before_source B C hs) (exec_before_fresh B C hC hs)
  ...
  | afterResidual =>
      let last := outerStepTo B (fullFinish (I := I) C)
        (exec_after_target B C hs) (exec_after_fresh B C hC hs)
