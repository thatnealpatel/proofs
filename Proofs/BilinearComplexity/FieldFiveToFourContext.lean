...
/-- The field with three elements used by the conditional contextual splice. -/
abbrev F3 := FieldFiveToFour.F3

variable {a b c : ℕ}


/-- Equal left-summand ranges of two equivalences give equal mapped right-summand ranges. -/
theorem range_comp_inr_eq_of_range_inl_eq
    {α β γ δ : Type*} (e f : α ⊕ β ≃ γ) (g : γ → δ)
    (hleft : Set.range (fun a : α => e (.inl a)) =
      Set.range (fun a : α => f (.inl a))) :
    Set.range (fun b : β => g (e (.inr b))) =
      Set.range (fun b : β => g (f (.inr b))) := by
  have right_compl (p : α ⊕ β ≃ γ) :
      Set.range (fun b : β => p (.inr b)) =
        (Set.range (fun a : α => p (.inl a)))ᶜ := by
    ext x
    constructor
    · rintro ⟨b, rfl⟩ ⟨a, ha⟩
      exact Sum.inl_ne_inr (p.injective ha)
    · intro hx
      obtain ⟨s, rfl⟩ := p.surjective x
      cases s with
      | inl a => exact False.elim (hx ⟨a, rfl⟩)
      | inr b => exact ⟨b, rfl⟩
  have hright : Set.range (fun b : β => e (.inr b)) =
      Set.range (fun b : β => f (.inr b)) := by
    rw [right_compl e, right_compl f, hleft]
  ext x
  constructor
  · rintro ⟨b, rfl⟩
...
theorem outerSource_eq_sourceState_of_sourceSource
...
theorem signedResidualSource_eq_singletonState_of_sourceSource
    {I : DisplayedF3PairTriple a b c} (B : CertifiedFiveToFour I)
    (hp : B.normalized.placement = .sourceSource) :
...
/-- In the occupied source/source schedule, the requested start is retained context union the
canonical signed residual source. -/
theorem requestStart_eq_residualContext_union_signedResidualSource_of_sourceSource_q_mem
    {I : DisplayedF3PairTriple a b c} (B : CertifiedFiveToFour I)
    (C : State F3 a b c) (hp : B.normalized.placement = .sourceSource)
    (hq : B.q ∈ C) :
    requestStart B C = stateUnion (residualContext B C) B.signedResidualSource := by
  rw [requestStart_eq_residualContext_union_residualSource_of_sourceSource_q_mem
      B C hp hq,
    residualSource_eq_signedResidualSource_of_sourceSource B hp]

/-- In the occupied source/source schedule, the requested finish is retained context union the
canonical signed residual target. -/
theorem requestFinish_eq_residualContext_union_signedResidualTarget_of_sourceSource_q_mem
    {I : DisplayedF3PairTriple a b c} (B : CertifiedFiveToFour I)
    (C : State F3 a b c) (hp : B.normalized.placement = .sourceSource)
    (hq : B.q ∈ C) :
    requestFinish B C = stateUnion (residualContext B C) B.signedResidualTarget := by
  rw [requestFinish_eq_residualContext_union_residualTarget_of_sourceSource_q_mem
      B C hp hq,
    residualTarget_eq_signedResidualTarget_of_sourceSource B hp]

/-- In the occupied source/source schedule, retained context is disjoint from the union of the
canonical signed residual endpoints. -/
theorem residualContext_disjoint_signedResidualEndpoints_of_sourceSource_q_mem
    {I : DisplayedF3PairTriple a b c} (B : CertifiedFiveToFour I)
    (C : State F3 a b c) (hp : B.normalized.placement = .sourceSource)
    (hq : B.q ∈ C)
    (hC : Disjoint C (stateUnion I.sourceState I.targetState)) :
  ...
  rw [residualContext_eq_of_sourceSource_q_mem B C hp hq,
    signedResidualSource_eq_singletonState_of_sourceSource B hp,
    signedResidualTarget_eq_targetState_of_sourceSource B hp]
  classical
  rw [Finset.disjoint_left]
  intro x hxContext hxEndpoints
  simp only [stateUnion, stateDifference, singletonState, Finset.mem_union,
    Finset.mem_sdiff, Finset.mem_singleton] at hxContext hxEndpoints
  rcases hxContext with hxOld | hxSource
  · rcases hxEndpoints with hxq | hxTarget
    · exact hxOld.2 hxq
    · exact Finset.disjoint_left.mp (context_target_disjoint B C hC)
        hxOld.1 hxTarget
  · rcases hxEndpoints with hxq | hxTarget
    · subst x
      exact q_not_mem_sourceState B hxSource
    · exact Finset.disjoint_left.mp I.source_target_disjoint hxSource hxTarget


/-- With the contraction atom absent in source/source placement, a context disjoint from the
original endpoints is retained unchanged. -/
theorem residualContext_eq_of_sourceSource_q_not_mem
    {I : DisplayedF3PairTriple a b c} (B : CertifiedFiveToFour I)
    (C : State F3 a b c) (hp : B.normalized.placement = .sourceSource)
    (hq : B.q ∉ C)
    (hC : Disjoint C (stateUnion I.sourceState I.targetState)) :
...
/-- In the absent source/source schedule, the requested start is retained context union the
canonical signed residual source. -/
theorem requestStart_eq_residualContext_union_signedResidualSource_of_sourceSource_q_not_mem
    {I : DisplayedF3PairTriple a b c} (B : CertifiedFiveToFour I)
    (C : State F3 a b c) (hp : B.normalized.placement = .sourceSource)
    (hq : B.q ∉ C)
    (hC : Disjoint C (stateUnion I.sourceState I.targetState)) :
  ...
  rw [requestStart, hs, outerSource_eq_sourceState_of_sourceSource B hp,
    ...
    residualContext_eq_of_sourceSource_q_not_mem B C hp hq hC,
    signedResidualSource_eq_singletonState_of_sourceSource B hp]

/-- In the absent source/source schedule, the requested finish is retained context union the
canonical signed residual target. -/
theorem requestFinish_eq_residualContext_union_signedResidualTarget_of_sourceSource_q_not_mem
    {I : DisplayedF3PairTriple a b c} (B : CertifiedFiveToFour I)
    (C : State F3 a b c) (hp : B.normalized.placement = .sourceSource)
    (hq : B.q ∉ C)
    (hC : Disjoint C (stateUnion I.sourceState I.targetState)) :
...
/-- In the absent source/source schedule, retained context is disjoint from the union of the
canonical signed residual endpoints. -/
theorem residualContext_disjoint_signedResidualEndpoints_of_sourceSource_q_not_mem
    {I : DisplayedF3PairTriple a b c} (B : CertifiedFiveToFour I)
    (C : State F3 a b c) (hp : B.normalized.placement = .sourceSource)
    (hq : B.q ∉ C)
    (hC : Disjoint C (stateUnion I.sourceState I.targetState)) :
    Disjoint (residualContext B C)
      (stateUnion B.signedResidualSource B.signedResidualTarget) := by
  rw [residualContext_eq_of_sourceSource_q_not_mem B C hp hq hC,
    signedResidualSource_eq_singletonState_of_sourceSource B hp,
    signedResidualTarget_eq_targetState_of_sourceSource B hp]
  classical
  rw [Finset.disjoint_left]
...
private theorem before_source_q {I : DisplayedF3PairTriple a b c}
    (B : CertifiedFiveToFour I) (C : State F3 a b c)
    (hs : schedule B C = .beforeResidual) :
    B.q ∈ CertifiedFiveToFour.outerSource B → B.q ∈ C := by
  intro hqS
...
private theorem before_target_q {I : DisplayedF3PairTriple a b c}
    (B : CertifiedFiveToFour I) (C : State F3 a b c)
    (hs : schedule B C = .beforeResidual) :
    B.q ∈ CertifiedFiveToFour.outerTarget B → B.q ∉ C := by
  intro hqT
...
private def execComposeWithLength {I : DisplayedF3PairTriple a b c}
    (B : CertifiedFiveToFour I) (C : State F3 a b c)
    (hC : Disjoint C (displayedCarrier I))
    (fourPath : StrictNativePath (requestStart B C) (requestFinish B C)) :
    { packet : FieldNativeExecutablePath.Packet F3 a b c //
      Path.length packet.path = Path.length fourPath + 1 } := by
  ...
  | beforeResidual =>
      have hsource : outerSource B ⊆ fullStart (I := I) C :=
        subset_transport (S := outerSource B) (D := fullStart (I := I) C)
          ...
          (source_subset_initial B C (before_source_q (I := I) B C hs))
      have hfresh : Disjoint (outerTarget B)
        ...
        exact disjoint_transport (outerTarget_eq_spec B) hrest
          (target_fresh_initial B C hCspec (before_target_q (I := I) B C hs))
      ...
      refine ⟨{
        ...
        path := .cons first tail }, ?_⟩
      have hlength : Path.length tail = Path.length fourPath := by
        exact length_castPath _ _ fourPath
      simp only [Path.length]
      omega
  | afterResidual =>
      have htarget : outerTarget B ⊆ fullFinish (I := I) C :=
        subset_transport (S := outerTarget B) (D := fullFinish (I := I) C)
          ...
          (target_subset_final B C (after_target_q (I := I) B C hs))
      have hfresh : Disjoint (outerSource B)
        ...
        exact disjoint_transport (outerSource_eq_spec B) hrest
          (source_fresh_final B C hCspec (after_source_q (I := I) B C hs))
      ...
      refine ⟨{
        start := fullStart (I := I) C
        finish := fullFinish (I := I) C
        path := FieldNativeExecutablePath.snoc head last }, ?_⟩
...
def execCompose {I : DisplayedF3PairTriple a b c}
    ...
    (hC : Disjoint C (displayedCarrier I))
    (fourPath : StrictNativePath (requestStart B C) (requestFinish B C)) :
    FieldNativeExecutablePath.Packet F3 a b c :=
  (execComposeWithLength B C hC fourPath).1
...
theorem length_execCompose {I : DisplayedF3PairTriple a b c}
    ...
    (hC : Disjoint C (displayedCarrier I))
    (fourPath : StrictNativePath (requestStart B C) (requestFinish B C)) :
    Path.length (execCompose B C hC fourPath).path = Path.length fourPath + 1 :=
  (execComposeWithLength B C hC fourPath).2
