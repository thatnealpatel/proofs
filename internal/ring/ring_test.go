...
func TestZ4OperationTables(t *testing.T) {
	testOperationTables(t, Z4,
		[][]int{{0, 1, 2, 3}, {1, 2, 3, 0}, {2, 3, 0, 1}, {3, 0, 1, 2}},
		[][]int{{0, 3, 2, 1}, {1, 0, 3, 2}, {2, 1, 0, 3}, {3, 2, 1, 0}},
		[][]int{{0, 0, 0, 0}, {0, 1, 2, 3}, {0, 2, 0, 2}, {0, 3, 2, 1}},
	)
}
...
func TestNegativeAndNoncanonicalValues(t *testing.T) {
	tests := []struct {
		...
		{"Z3 sub", Z3.Sub(-7, 8), 0},
		{"Z3 mul", Z3.Mul(-7, 8), 1},
		{"Z4 normalize negative", Z4.Normalize(-7), 1},
		{"Z4 normalize positive", Z4.Normalize(10), 2},
		{"Z4 add", Z4.Add(-7, 10), 3},
		{"Z4 sub", Z4.Sub(-7, 10), 3},
		{"Z4 mul", Z4.Mul(-7, 10), 2},
	}
...
func TestIntegerExtremes(t *testing.T) {
	...
	tests := []struct {
		...
		{"Z4 normalize MinInt", Z4.Normalize(minInt), 0},
		{"Z4 normalize MaxInt", Z4.Normalize(maxInt), 3},
		{"Z4 add extremes", Z4.Add(minInt, maxInt), 3},
		{"Z4 sub extremes", Z4.Sub(minInt, maxInt), 1},
		{"Z4 mul extremes", Z4.Mul(minInt, maxInt), 0},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if test.got != test.want {
...
func TestRingValidity(t *testing.T) {
	for _, r := range []Ring{Z2, Z3, Z4} {
		if !r.Valid() {
			t.Errorf("Ring(%d).Valid() = false", r)
		}
	}
	for _, r := range []Ring{-3, 0, 1, 5} {
		if r.Valid() {
